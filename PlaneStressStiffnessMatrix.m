...
function[]=PlaneStressStiffnessMatrix
...
for iel=1:mesh.ne
    ...
    for ig=1:size(quadrature.points,1)
        ...
        B_pf=zeros(3,shape_order*2);
        %B_pt=zeros(2,shape_order*2);
        ...
        for ib=1:shape_order
            ...
            B_pf(:,ib*2-1:ib*2)=temp;
    
%             temp=[Nd(ib,1) shapeFunction.fun(ib) 0
%                   Nd(ib,2) 0 shapeFunction.fun(ib)];
%             B_pt(:,ib*3-2:ib*3)=temp;  
        end
        KElem=KElem+quadrature.weights(ig)*(beta*B_pf'*D_f*B_pf)*det(jacobian.matrix);
    end
